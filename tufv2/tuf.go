...
// the Repo instance.
type Repo struct {
	Root      *data.SignedRoot      // fully desreialized root metadata
	Snapshot  *data.SignedSnapshot  // snapshot metadata
	Timestamp *data.SignedTimestamp // timestamp metadata

	// fully deserialized top level targets metadata and the root of the delegations tree
	// (pointers to fully deserialized delegations are in the roles specified by this
	// top level targets metadata)
