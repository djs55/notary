...
// GetBaseRole gets one of the base roles from the root metadata
func (r SignedRoot) GetBaseRole(name string) (BaseRole, error) {
	roleData, ok := r.Signed.Roles[name]
	if !ok {
		return BaseRole{}, ErrMissingMeta{role: name}
	}
